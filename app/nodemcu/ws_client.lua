...
function websocket.createClient()
    local client = {
        ...
        on_sent = nil,     -- on_close(client, received_headers)
        ...
        send = function (self, data, opcode)
            ...
            _G.printLine("[client] Sending data: "..data.."\r\n")
            local params = {
                payload = data,
                opcode = opcode,
            }
        ...
        receive = function (self, chunk)
            ...
            _G.printLine("[client] Received opcode="..tostring(opcode)..", payload="..tostring(payload)..", extra="..tostring(extra))
            _G.printLine("[client] Free memory: "..tostring(node.heap()))
            if not opcode then
                -- print("[client] Not found opcode")
                _G.printLine("[client] Not found opcode")
            elseif opcode == 0x0 then
                -- print("[client] Not full frame")
                _G.printLine("[client] Not full frame")
            ...
            if opcode == 9 then         -- ping
                ...
                _G.printLine("[client] Received PING from the server")
                self:send("Pong", 0xA)
                return
            elseif opcode == 0xA then       -- pong
                -- print("[client] Received PONG from the server")
                _G.printLine("[client] Received PONG from the server")
            ...
            elseif opcode == 0x8 then       -- close
                ...
                _G.printLine("[client] Received CLOSE from the server")
                self:close()
                return
            end
        ...
        ping = function (self)
            ...
            _G.printLine("[client] Send PING to the server")
            self:send("Ping", 0x9)
        end,

        sendHandshake = function (self, host, path)
            -- print("[client] Handshaking")
            _G.printLine("[client] Handshaking")
            local params = {
                socket = self.socket,
                host = host,
                path = path,
        ...
        readHandshake = function (self, result)
            ...
            _G.printLine("[client] Read handshake: "..result)
        ...
        connect = function (self, connection_string, timeout)
            ...
            _G.printLine("[client] Connect to "..connection_string)
            ...
            self.socket:on("sent", function (sk, c) self:sent() end)

            -- print("[client] Connect with host "..host.." and port "..port)
            _G.printLine("[client] Connect with host "..host.." and port "..port)
        ...
        close = function (self)
            ...
            _G.printLine("[client] Close connection")
            if self.socket then
                self.socket:close()
            end
            self.socket = nil
